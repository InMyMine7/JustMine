import requests  # Mengimpor library requests untuk melakukan permintaan HTTP
import os  # Mengimpor modul os untuk berinteraksi dengan sistem operasi
import json  # Mengimpor modul json untuk mengelola konfigurasi
import re
import time
import platform
from colorama import Fore, init  # Mengimpor Fore dan init dari colorama untuk mengatur warna teks di terminal
from lib.colors import r, g, wh, res, bg_red  # Sesuaikan dengan struktur Anda
...
class ZoneH_Grabber:
    ...
    def save_config(self):
        ...
        with open("lib/config.json", "w") as file:
    ...
    def Grab(self):
        if not self.ResultFileName:
            self.ResultFileName = self.input(f'{wh}[{g}+{wh}] Input Result File Name: [Ex: Result.txt]=> ') or 'list0.txt'
...
class ZoneXSecGrabber:
    ...
    def grab(self):
        if not self.ResultFileName:
            self.ResultFileName = input(f'{wh}[{g}+{wh}] Enter Result File Name (e.g., Result.txt): {res}') or 'list0.txt'
            print(f'{wh}[{g}+{wh}] Results will be stored in: {self.ResultFileName}\n\nStarting...')
